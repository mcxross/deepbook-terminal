...
fn main() {
    ...
    let candles = match args.mode {
        CandlesRetrievalMode::Stdin => parse_candles_from_stdin(),
        CandlesRetrievalMode::CsvFile => {
            let filepath = args.file_path.expect("No file path provided.");
    ...
    if let Some(chart_name) = args.chart_name {
        chart.set_name(chart_name);
    }

    if let Some(bear_color) = args.bear_color {
        let (r, g, b) = hexa_to_rgb(bear_color);
        chart.set_bear_color(r, g, b);
    }

    if let Some(bull_color) = args.bull_color {
