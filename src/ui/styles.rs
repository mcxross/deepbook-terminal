...
use crate::data::{Market, PriceColorMode};
use ratatui::{
...
pub fn up_color(val: Ordering) -> Color {
    ...
    match val {
        Ordering::Less => match price_color_mode() {
            PriceColorMode::RedUp => green,
            PriceColorMode::GreenUp => red,
        },
        Ordering::Equal => Color::Reset,
        Ordering::Greater => match price_color_mode() {
            PriceColorMode::RedUp => red,
            PriceColorMode::GreenUp => green,
        },
    }
}

#[inline]
pub fn price_color_mode() -> PriceColorMode {
    // Default to GreenUp mode (green for up, red for down - China mainland convention)
    // TODO: Read from user settings
    PriceColorMode::GreenUp
}
...
pub fn bull_bear() -> (Style, Style) {
    ...
    match price_color_mode() {
        PriceColorMode::RedUp => (red, green),
        PriceColorMode::GreenUp => (green, red),
    }
}
