...
bitflags! {
    ...
    pub struct DirtyFlags: u32 {
        ...
        /// Pool detail view needs update (individual pool data)
        const POOL_DETAIL = 0b0000_0010;
        /// Index carousel needs update (top market indexes)
        const INDEXES = 0b0000_0100;
        /// Help popup needs update
        const POPUP_HELP = 0b0000_1000;
        /// Search popup needs update
        const POPUP_SEARCH = 0b0001_0000;
        /// Watchlist group selector popup needs update
        const POPUP_WATCHLIST = 0b0010_0000;
...
impl DirtyFlags {
    ...
    pub fn mark_quote_update(mut self) -> Self {
        self.insert(Self::WATCHLIST | Self::POOL_DETAIL | Self::INDEXES | Self::STATUS_BAR);
        self
    }
    ...
    pub fn mark_depth_update(mut self) -> Self {
        self.insert(Self::POOL_DETAIL | Self::DEPTH);
        self
    }
...
mod tests {
    ...
    fn test_dirty_flags() {
        ...
        let flags = DirtyFlags::WATCHLIST | DirtyFlags::POOL_DETAIL;
        assert!(flags.needs_render());
        assert!(flags.contains(DirtyFlags::WATCHLIST));
        assert!(flags.contains(DirtyFlags::POOL_DETAIL));
    }
